...
module.exports = class MessageListener extends Listener {
	...
	async exec(message) {
		...
		if (
			...
			if (await this.client.sql.get(`firstime: ${message.author.id}`)) {
				await this.client.sql.delete(`firstime: ${message.author.id}`);
				await message.send(
		...
		if (
			...
			message.send(
				this.client
					.embed()
					.setDescription(
						'Try our `t)pc` command to paste your code in a source bin from Discord!'
					)
					.setAuthor(
