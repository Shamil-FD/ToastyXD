...
module.exports = class LockDownCommand extends Command {
  ...
  async exec(message) {
    ...
    if (this.client.config.lockingMode === true)
      msg.edit({
        embeds: [
          this.client.tools.embed().setDescription("There's already a lockdown process running!").setColor('RED'),
        ],
      });
    ...
    let channels3 = channels.filter((c) =>
      ['709043328682950716', '709043831995105360', '752983872068780204', '752983993720504340'].includes(c.id),
    );
    ...
    try {
        ...
        await message.guild.channels.cache
          ...
          .send({
            embeds: [
              this.client.tools
                .embed()
                .setDescription(`The server has been unlocked by <@${message.author.id}>`)
                .setColor('GREEN'),
            ],
          });
        return msg.edit({
          embeds: [this.client.tools.embed().setDescription(`Successfully unlocked the server!`).setColor('GREEN')],
        });
      } else {
        ...
        await message.guild.channels.cache
          ...
          .send({
            embeds: [
              this.client.tools
                .embed()
                .setDescription(`The server has been locked by <@${message.author.id}>`)
                .setColor('RED'),
            ],
          });
        return msg.edit({
          embeds: [this.client.tools.embed().setDescription(`Successfully locked the server!`).setColor('GREEN')],
        });
      }
    } catch (e) {
      console.log(e);
      return msg.edit({
        embeds: [
          this.client.tools.embed().setDescription(`There was an error!\n\`\`\`\n${e.stack}\`\`\``).setColor('RED'),
        ],
      });
    }
  }
