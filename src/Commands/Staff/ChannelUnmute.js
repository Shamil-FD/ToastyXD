...
module.exports = class ChannelUnmuteCommand extends Command {
    ...
    async execSlash(message) {
		...
		if (!doc) return message.editReply(this.client.embed().setDescription('They are already unmuted.'));
        ...
        if (hasError === false) {
            message.editReply(this.client.embed().setDescription(`Unmuted ${user} in this channel for ${reason}`).setAuthor(message.member?.user?.username, message.member?.user?.displayAvatarURL({ dynamic: true })))
        } else {
            message.editReply(this.client.embed().setDescription(`${user} was already unmuted in this channel, but I deleted the document from my database.`).setAuthor(message.member?.user?.username, message.member?.user?.displayAvatarURL({ dynamic: true })))
        }
        
        return message.guild.channels.cache.get(this.client.config.StaffReportChnl).send(this.client.embed().setTitle('User Unmuted').setAuthor(`${this.client.arrow} Moderator: ${message.member?.user?.username}`, message.member?.user?.displayAvatarURL({ dynamic: true })).addField(this.client.arrow + ' **Victim**:', `${user} || ${user?.id}`, true).addField(this.client.arrow + ' **Reason**:', reason, true).addField(this.client.arrow + ' **Channel**:', message.channel, true))
    }
};
