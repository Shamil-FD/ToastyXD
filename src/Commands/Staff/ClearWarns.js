...
module.exports = class ClearWarnsCommand extends Command {
    ...
    async execSlash(message) {
        ...
        if (message.member.id === message.options[0]?.member?.id) return message.reply("You can't do that.", { ephemeral: true });
        message.defer()
        
		let docs = await this.client.models.warn.find({ user: message.options[0]?.member?.id });
        if (!docs.length) return message.editReply("They don't have any logged warns.");
        
        await message.editReply("Please wait..");
        try {
            await this.client.models.warn.deleteMany({ user: message.options[0]?.member?.id });
