...
module.exports = class LeaveCommand extends Command {
        ...
        await message.guild.channels.cache.get('757169784747065364').send({
          embeds: [
            client
              ...
              .setDescription(
                `${message.author} is now on leave.\nStart Date: ${dayjs(start).format(
                  'DD MMMM YYYY',
                )} - End Date: ${dayjs(end).format('DD MMMM YYYY')} - Reason: ${reason}`,
              )
  ...
  async execSlash(message) {
    ...
    if (message.options[0]?.name === 'end') {
      ...
      if (!doc)
        return message.editReply({ embeds: [this.client.embed().setDescription("You weren't on leave, dummy.")] });
      await doc.delete();
