...
module.exports = class AFKCommand extends Command {
	...
	async exec(message, { mute, reason }) {
		...
		if (!doc) {		
            if (mute) {
                mute = await ms(mute);
                if (!mute) return message.send({ embeds: { description: 'Invalid time provided.' }});
    ...
    async execSlash(message) {
		...
		if (!doc) {		
            if (mute) {
                mute = await ms(mute);
                if (!mute) return message.editReply({ ephemeral: true, embeds: [{ description: 'Invalid time provided.' }]});
                await message.member.roles.add(await message.guild.roles.fetch(this.client.config.MutedRole))                
            }
            await new afk({
				user: message.author.id,
