...
module.exports = class DictionaryCommand extends Command {
			...
			fetched.forEach((f) => {
				result.push(
					f.meanings
						.map(
							(m) =>
								`${this.client.arrow} **Part Of Speech**: ${
									m.partOfSpeech
								}\n${m.definitions
									.map(
										(d) =>
	...
	async execSlash(message) {
		let word;
		let urban;
		if (message.options[0].name === 'normal')
			word = message.options[0]?.options[0]?.value;
		else if (message.options[0].name === 'urban')
			urban = message.options[0]?.options[0]?.value;
