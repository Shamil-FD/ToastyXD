...
module.exports = class ResetStrikesCommand extends Command {
	...
	async exec(message, { user }) {
		...
		if (!user)
			return message.reply(this.client.embed().setDescription('Invalid user!'));

		let doc = await staff.findOne({ user: user.id });
		if (!doc)
			return message.reply(
				this.client.embed().setDescription("They don't have any strikes.")
			);

		doc.strikes = 0;
		await doc.save();
		return message.reply(
			this.client.embed().setDescription('POOF! Begone strikes!')
		);
	}
};
