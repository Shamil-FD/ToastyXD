...
module.exports = class EditmsgCommand extends Command {
  ...
  async exec(message, { channel, id, title, content }) {
    ...
    if (!channel) return message.send("t)editmsg #Channel MESSAGEID <optional `title: title-here`> DESCRIPTION");
    if (!id) return message.send("t)editmsg #Channel MESSAGEID <optional `title: title-here`> DESCRIPTION");
    if (!content) return message.send("t)editmsg #Channel MESSAGEID <optional `title: title-here`> DESCRIPTION");
