...
module.exports = class StrikeCommand extends Command {
  ...
  async exec(message, { user, reason }) {
    ...
    if (!user) return message.send({ embeds: { description: "No member mention provided." } });
    ...
    return message.send({
      embeds: { description: `Striked ${user} for ${reason}. They now have ${doc.strikes} strike(s).` },
    });
  }
};
