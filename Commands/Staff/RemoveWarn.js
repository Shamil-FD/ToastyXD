...
module.exports = class RemoveWarnCommand extends Command {
  ...
  async exec(message, { id }) {
    if (!id || id.isNaN)
      return message.send(this.client.embed().setDescription("Huh.. Remove nothing?").setColor("RED"));

    let doc = await warn.findOne({ id: id });
    if (!doc) return message.send(this.client.embed().setDescription(`Case ID: ${id} doesn't exist!`));
