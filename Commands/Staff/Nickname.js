...
module.exports = class NicknameCommand extends Command {
  ...
  async exec(message, { ops, user }) {
    ...
    ops = ops.trim();
    if (!user) return message.send(client.embed().setDescription("Are you sure that you provided a user?"));

    user = await message.getMember(user);
    if (!user) return message.send(client.embed().setDescription("In-invalid u-uuserrr! err!"));

    if (user.roles.cache.get(this.client.config.StaffRole))
      return message.send(
        client.embed().setDescription("Moderator changing a Moderator's nickname? That's a cap (whatever cap means)")
      );
    ...
    await user.setNickname(nick).catch(() => {
      stop = true;
      return message.send(client.embed().setDescription("I couldn't change their nickname, err!"));
    });
    if (stop == true) return;
    return message.send(client.embed().setDescription(`Changed ${user}'s nickname`));
  }
};
