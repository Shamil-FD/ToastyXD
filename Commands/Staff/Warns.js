...
module.exports = class WarningsCommand extends Command {
  ...
  async exec(message, { user }) {
    ...
    if (!user) return message.send(this.client.embed().setDescription(`I couldn't find \`${user}\``));

    let doc = await warn.find({ user: user.id });
    if (!doc.length) return message.send(this.client.embed().setDescription(`${user} hasn't been warned before.`));
    ...
    let msg = await message.send({ embeds: { description: "React to delete these message(s).", color: "GREEN" } });
    try {
