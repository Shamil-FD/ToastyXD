		...
		if (!user)
			return message.send(
				this.client
					.embed()
					.setDescription(
						"You can't warn anybody without providing me an user."
					)
			);
		if (!reason)
			return message.send(
				this.client
					.embed()
					.setDescription("You can't warn someone without a valid reason.")
			);
		if (user.id === message.member.id)
			return message.send(
				this.client.embed().setDescription("You can't warn yourself!")
			);
		if (user.user.bot)
			return message.send(
				this.client.embed().setDescription("You can't warn one of my kind.")
			);
		message.delete();
		...
		if (docs.length) {
			...
			try {
				...
				if (collected.first().emoji.name === this.client.tick) {
					...
					return message.send({
						embeds: { description: 'Verbal Warn cancelled.', color: 'GREEN' },
					});
				} else
					return message.send({
						embeds: { description: 'Verbal Warn cancelled.', color: 'RED' },
					});
			} catch (e) {
				(await msg.deleted) ? null : msg.delete();
				await msgs.forEach(async (m) => {
