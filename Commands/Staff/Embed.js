...
module.exports = class EmbedCommand extends Command {
  ...
  async exec(message, { chnl, title, desc, ping }) {
    ...
    if (ping) {
      chnl.send("@everyone", embed.setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true })));
    } else {
      chnl.send(embed.setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true })));
    }
    message.delete();
