  ...
  async exec(message, { bio }) {
    ...
    if (!bio)
      return message.send(message.author, {
        embeds: { description: "Didn't quite catch that.. Can you give me a description of yourself?", color: "RED" },
      });
    let doc = await staff.findOne({ user: message.author.id });
    doc.desc = bio;
    await doc.save();
    return message.send(message.author, { embeds: { color: "GREEN", description: `Set \`${bio}\` as your bio.` } });
  }
};
