...
module.exports = class ChannelUnmuteCommand extends Command {
  ...
  async exec(message, { user, reason }) {
    ...
    if (!user)
      return message.send({ embeds: { color: "RED", description: "Proper usage: `t)channelunmute @User [reason]`" } });

    if (!reason) return message.send({ embeds: { description: "I don't see any reason provided.", color: "RED" } });
