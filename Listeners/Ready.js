      ...
      cron.schedule(`* * * * *`, async () => {
        ...
        if (unbans.length) {
          unbans.forEach(async unbandoc => {
            ...
            if (bannedUser) {
              await sal.members.unban(unbandoc.user, unbandoc.reason + ` - ${unbandoc.mod}`);
              await sal.channels.cache
                ...
                .send(
                  this.client
                    ...
                    .setAuthor(`Unbanned ${bannedUser.user.tag}`, bannedUser.user.displayAvatarURL())
                );
          ...
          docs.forEach(async d => {
            ...
            if (time - (Date.now() - date) < 0) {
              ...
              try {
                ...
                await models.chnlmute.findOneAndDelete({ user: d.user, chnl: d.chnl });
                hasError = true;
              }
              if (hasError === false) {
                await sal.channels.cache
        ...
        if (no.length) {
          no.forEach(async n => {
            ...
            if (StrikeDoc && StrikeDoc.strikes >= 3) {
              await sal.channels.cache.get("805154766455701524").send(`<@${n}> has ${StrikeDoc.strikes} strikes now.`);
              setTimeout(async () => {
