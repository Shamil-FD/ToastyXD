		...
		constructor(client, data, channel) {
			...
			this.send = async (args, options = {}) => {
				...
				if (options.embeds || args.embeds) {
					...
					options.embeds.color
						? embed.setColor(options.embeds.color)
						: embed.setColor('#ffb600');
					...
					if (options.embeds.description.length > 2048) {
						...
						if (splitted.length == 1) {
							return this.channel.send(
								args.embeds ? '' : args,
								embed.setDescription(splitted[0])
							);
						} else {
						...
						options.embeds.description
							? embed.setDescription(options.embeds.description)
							: null;
